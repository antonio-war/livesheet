...
def image_to_text(request):
    ...
    model = "model/semantic_model.meta"
...
def resize(image, height):
    width = int(float(height * image.size[0]) / image.size[1])
    sample_img = image.resize((width, height), resample=0)
    return sample_img
